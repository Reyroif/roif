...
def ___login___():
    os.system('clear')
    print(___logo___)
    print(f"{B}[{P}•{B}]{P} copas ken Cookie Instagram, mun te bisa neang Cookie Instagram Ketik {M}[{P}Open{M}]{P}\n")
...
def ___follow___():
    try:
        ...
        ___teks = random.choice(['Hallo Bang ??','Hai Bang Apa Kabar ??','Izin Pake Scriptnya ??','Mantap Bang ??','Programmer Bang ??','Salam Kenal Bang ??','I Love You ??'])
